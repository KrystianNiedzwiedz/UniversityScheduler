...
export default class GrantCourseForm extends React.Component
{
    state = {
        id: '',
        userId: uuidv4(),
    ...
    async grantCourse()
    {
        fetch(`${window.INVENTORY_COURSES_API_URL}`, {
            method: 'post',
            headers: {
            ...
            .then(async response =>
            {
                if (!response.ok)
                {
                    ...
                    throw new Error(`Could not grant the course: ${errorData.title}`);
                }
